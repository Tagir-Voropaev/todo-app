    ...
    return (
        <nav className={s.nav}>
            ...
            <div className={s.navAuth}>
                <NavLink to="/login" className={({ isActive }) => `${s.navAuthLink} ${isActive && s.navAuthActive}`}>Войти</NavLink>
                <NavLink to="/register" className={({ isActive }) => `${s.navAuthLink} ${isActive && s.navAuthActive}`} >Регистрация</NavLink>
            </div>
        </nav>
    )
}
